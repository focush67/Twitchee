"use client";

import { ChatSidebarContext, ChatType } from "@/context/chat-sidebar-context";
import {
  useChat,
  useConnectionState,
  useRemoteParticipant,
} from "@livekit/components-react";
import { ConnectionState } from "livekit-client";
import { useContext, useEffect, useMemo, useState } from "react";
import { useMediaQuery } from "usehooks-ts";
import ChatHeader, { ChatHeaderSkeleton } from "./chat-header";
import ChatForm, { ChatFormSkeleton } from "./chat-form";
import ChatList, { ChatListSkeleton } from "./chat-list";
import Community from "../community-related-components/community";

interface ChatProps {
  viewer: string;
  color: string;
  host: string;
  hostId: string;
  isFollowing: boolean;
  isChatEnabled: boolean;
  isChatDelayed: boolean;
  isChatPrivate: boolean;
}

const Chat = ({
  viewer,
  color,
  host,
  hostId,
  isFollowing,
  isChatEnabled,
  isChatDelayed,
  isChatPrivate,
}: ChatProps) => {
  const matches = useMediaQuery("(max-width:1024px");
  const { type, onExpand } = useContext(ChatSidebarContext) || {};
  const connectionState = useConnectionState();
  const participant = useRemoteParticipant(hostId);

  const isOnline = participant && connectionState === ConnectionState.Connected;

  const isHidden = !isChatEnabled || !isOnline;
  const [value, setValue] = useState("");
  const { chatMessages: messages, send } = useChat();

  useEffect(() => {
    if (matches) {
      onExpand?.();
    }
  }, [matches, onExpand]);

  const reversedMessages = useMemo(() => {
    return messages.sort((a, b) => b.timestamp - a.timestamp);
  }, [messages]);

  const onSubmit = () => {
    if (!send) return;

    send(value);
    setValue("");
  };

  const onChange = (value: string) => {
    setValue(value);
  };

  return (
    <div className="flex flex-col bg-background border-l border-b pt-0 h-[calc(100vh-80px)]">
      <ChatHeader />
      {type === ChatType.CHAT && (
        <>
          <ChatList
            messages={reversedMessages}
            hidden={isHidden}
            color={color}
          />
          <ChatForm
            onSubmit={onSubmit}
            onChange={onChange}
            isHidden={isHidden}
            isPrivate={isChatPrivate}
            value={value}
            isDelayed={isChatDelayed}
            isFollowing={isFollowing}
          />
          <p>Chat Mode</p>
        </>
      )}
      {type === ChatType.COMMUNITY && (
        <>
          <Community viewer={viewer} host={host} isHidden={isHidden} />
        </>
      )}
    </div>
  );
};

export default Chat;

export const ChatSkeleton = () => {
  return (
    <div className="flex flex-col border-l border-b pt-0 h[calc(100vh - 80px)] border-2">
      <ChatHeaderSkeleton />
      <ChatListSkeleton />
      <ChatFormSkeleton />
    </div>
  );
};
